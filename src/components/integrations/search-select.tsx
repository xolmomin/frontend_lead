...
/**
 * Production-style searchable select (port of the prod `Select` chunk):
 * button trigger + dropdown with search box, keyboard navigation and
...
export function SearchSelect({
  ...
  const updatePosition = useCallback(() => {
    ...
    setPosition(below < 300 && above > below ? "top" : "bottom");
  }, []);
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      case "ArrowDown":
        ...
        if (open) {
          setHighlighted((index) =>
            index < filtered.length - 1 ? index + 1 : index,
          );
        } else {
          toggleOpen();
        }
      ...
      case "ArrowUp":
        event.preventDefault();
        setHighlighted((index) => (index > 0 ? index - 1 : index));
        break;
    }
  };

  const pick = (optionValue: string, optionDisabled?: boolean) => {
  ...
  const renderOption = (option: SearchSelectOption, index: number) => {
    ...
    return (
      <div
        ...
        onClick={() => pick(option.value, isDisabled)}
                        ...
                        <div key={groupIndex}>
                          ...
                          {group.options.map((option, optionIndex) => {
                            const flatIndex = groups
                              .slice(0, groupIndex)
                              .reduce(
                                (sum, other) => sum + other.options.length,
                                optionIndex,
                              );
